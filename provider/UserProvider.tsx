...
export function UserProvider({ children }: { children: ReactNode }) {
    ...
    const getProfile=async () => {
        ...
        if (error) {
            ...
            let rol=data[0].role==='admin'
            setUserInfo({ ...userInfo, profile: data[0], role: data[0].role, userId: userInfo.session.user.id, isAdmin: rol })
    ...
    return (
        <UserContext.Provider value={{ ...userInfo, loading, saveProfile, getProfile }}>
            {children}
